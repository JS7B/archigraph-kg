...
## 2026-07-12 图谱体验 QA：本地优先渲染与证据优先详情

- 做了什么：为图谱页面补齐社区概览、局部图展开、请求失败、证据详情和键盘焦点的用户行为测试，并记录本地优先加载流程。
- 这是什么：local-first rendering 指先请求有边界的社区和局部子图，再回退到完整实体列表；稳定布局指切换筛选或展开时保留已经看到的节点位置；evidence-first detail 指详情面板优先展示关系方向、置信度、证据 chunk 和文档来源。
