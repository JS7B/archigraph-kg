...
- 踩了什么坑：不能直接对整段文本做正则替换，否则示例代码里的 `[0]` 或 `[99]` 会被误删；code span 还允许跨行，必须把 fenced code 之外的连续文本作为整体处理。角标数字也不能无界传给 `int()`，否则超长模型输出会触发 Python 的数字转换限制；改用规范化数字字符串与可用编号比对，既不改解释器全局限制，也不会让 Run 因无效角标失败。
