...
- 踩了什么坑：不能直接对整段文本做正则替换，否则示例代码里的 `[0]` 或 `[99]` 会被误删；code span 还允许跨行，且普通文本中奇数反斜杠会转义 opening backtick，必须先按 delimiter 语义隔离。角标数字也不能无界传给 `int()`，否则超长模型输出会触发 Python 的数字转换限制；改用规范化数字字符串与可用编号比对。代码块采用 fenced、四空格和 tab 的有界扫描，不承诺解析复杂列表嵌套的完整 CommonMark 语法。
