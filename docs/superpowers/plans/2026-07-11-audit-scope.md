...
Add exact `BRANCH_SCOPES` for `feat/kg-parsing`, `feat/kg-extraction`, `feat/kg-resolution`, `feat/kg-community-api`, and `feat/graph-experience`. `commands_for_paths` must select the narrow backend command for parsing, extraction, graph/router, and frontend paths. A path outside the exact branch scope must still block.
