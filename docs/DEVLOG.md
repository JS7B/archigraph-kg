...
## 2026-07-20 为 Agentic RAG 正确性波次建立分支门禁

- 做了什么：为历史指代、引用校验、会话原子写入和 canonical QA 扩展四条固定 worktree 注册精确文件白名单，并为每条分支接入 Python 编译检查与无外部服务的聚焦 pytest。
- 这是什么：分支门禁是主仓库评审前的机械质量护栏；它先确认工人只改自己拥有的文件，再用 compileall 检查语法、用 fake/mock 测试检查确定性行为。
- 为什么需要：四条优化会先后修改 QA Agent、运行任务和会话存储等相邻文件；没有分支范围和固定验证入口时，工人容易把跨波次改动混在一起，或把真实模型、个人 Neo4j 是否可用误当成代码正确性。
- 为什么这么做：生产和测试文件都使用精确路径，P/Q/S/R 分别运行自己的最小用例；真实 LLM 与真实 Neo4j 验证保留给主仓库终审，因为分支即时门禁需要在新 worktree 没有 `.env` 时仍可复现。
- 踩了什么坑：执行规划要求保留 compile/diff 检查，但基线 gate 只有三种 `git diff --check`；因此补了从 `backend/` 执行的 compileall，并明确它只检查语法、会生成被忽略的字节码缓存，不能冒充业务测试或安全边界。
...
- 踩了什么坑：同一条消歧判断不能克隆成“每个 mention chunk 都是消歧证据”；持久化边只保存一个真实、已由 `MENTIONS` 连接的 evidence chunk，其余来源继续从源图动态查询。规范名也不能设唯一约束，否则同名异义实体永远无法人工拆分；review 必须保存稳定候选列表，而不是只留一句“有歧义”。
...
- 这是什么：有限并发用固定大小线程池重叠远程 LLM 等待，但结果仍按文档顺序进入合并；人工复核基线则保留每个指标的分子、分母和覆盖率，把“未匹配标注”与“人工确认错误”分开。
- 为什么需要：串行 chunk 调用会累计网络等待，而只看召回率和连通分量无法判断图谱杂乱究竟来自误抽取、重复实体还是展示契约。
- 为什么这么做：P、Q 修改目录彼此独立，适合并行；每个工人只在固定分支提交，主仓库负责规格、门禁、退回补测和合并。并发上限先限定为单文档默认 3，不把全局服务商限流和重试改造混入本批次。
- 踩了什么坑：首次 runs 门禁从仓库根执行时继承了顶层 Neo4j 自动夹具，worktree 没有 `.env` 会在单元测试收集前因空 URI 失败。最终让命令从 `backend/` 执行、用 runs-local `confcutdir`，并明确排除既有 live Neo4j 会话持久化用例，使无服务门禁真正可复现。
